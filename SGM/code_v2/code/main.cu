...
int loadImage(std::string filename, DeviceImage *d_img)
{
	...
	int r = input_bgr.rows;
	int c = input_bgr.cols;
	std::cout << "Image size ="<<r*c<<std::endl;
...
int main(int argc, char **argv)
{
	...
	et = rdtsc();
	et = ((et-st)*1)/1;
	latency = (double)et/20;
	double perf = 0;
	std::cout <<"Cycles = "<<latency<<std::endl;
	// write disparity map to file
